...
- `GET /__api__/v1/content` → list user's content items; filtered to `app_role == "owner" | "editor"` in `handle_app_event`
- `GET /__api__/v1/content/{guid}/environment` → returns **names only** (values are redacted by Connect); `EnvVar.value` is always `None` after fetch
...
`get_env_vars` handles three response shapes Connect has used across versions: plain `["NAME", ...]`, `[{name, value}]`, and `{"KEY": "VALUE"}` map.

**Corporate TLS**: set `SSL_CERT_FILE` (single PEM) or `SSL_CERT_DIR` (directory of `*.pem` files) to inject custom CA certificates into the reqwest client.
...
On startup, `check_auto_refresh` triggers `trigger_fetch` if `config.last_refresh` is absent or > 24 hours old. `last_refresh` is written (RFC3339) after each successful `ProjectsFetched` event.
